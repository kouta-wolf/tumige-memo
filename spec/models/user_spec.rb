...
RSpec.describe User, type: :model do
  ...
  describe "異常系" do
    ...
    it "パスワードが長い場合は無効であること" do
      ...
      user.valid?
      expect(user.errors[:password]).to include(I18n.t('errors.messages.too_long', count: 20))
    end

    it "パスワードと確認用パスワードが一致しない場合は無効であること" do
      user = build(:user, password: "password", password_confirmation: "miss")
      user.valid?
      expect(user.errors[:password_confirmation]).to include(I18n.t('errors.messages.confirmation', attribute: User.human_attribute_name(:password)))
    end
  end
